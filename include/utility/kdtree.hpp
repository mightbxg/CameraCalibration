...
public:
     ...
     */
    std::vector<int> knnSearch(const PointT& query, int k, std::vector<double>* dists = nullptr) const
    {
        KnnQueue queue(k);
        knnSearchRecursive(query, root_, queue, k);

        std::vector<int> indices(queue.size());
        std::vector<double> distances(queue.size());
        for (size_t i = 0; i < queue.size(); i++) {
            distances[i] = queue[i].first;
            indices[i] = queue[i].second;
        }

        if (dists)
            std::swap(*dists, distances);
